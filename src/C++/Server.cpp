...
private:
    ...
    std::map<std::string, std::set<std::string>> published_files; // filename -> usernames
    std::map<std::string, std::set<std::string>> user_files;      // username -> filenames
...
public:
    ...
    void log(const std::string &msg) {
        ...
        std::cout << "[" << std::put_time(&now_tm, "%Y-%m-%d %H:%M:%S") << "] " << msg << std::endl;
    }
    ...
    void handle_hbt(const std::string &username, const sockaddr_in &client_addr) {
        std::lock_guard<std::mutex> lock(mtx);
    ...
    void handle_sch(const std::string &username, const std::string &substring, const sockaddr_in &client_addr) {
        ...
        for (const auto &[file, users] : published_files) {
            if (file.find(substring) != std::string::npos) files_found.insert(file);
        }
        std::string response = "OK " + std::to_string(files_found.size());
        for (const auto &file : files_found) response += " " + file;
        send_response(response, client_addr);
    }
    ...
    void handle_get(const std::string &username, const std::string &filename, const sockaddr_in &client_addr) {
        std::lock_guard<std::mutex> lock(mtx);
        std::string response = "ERR";
        if (published_files.count(filename) && !published_files[filename].empty()) {
            ...
            if (response == "ERR") response = "ERR No active peer has file";
        } else {
    ...
    void start() {
        ...
        while (running) {
            ...
            if (req_type == "AUTH" && tokens.size() > 2) handle_auth(username, tokens[2], client_addr);
            else if (req_type == "HBT") handle_hbt(username, client_addr);
            else if (req_type == "LAP") handle_lap(client_addr);
            else if (req_type == "PUB" && tokens.size() > 2) handle_pub(username, tokens[2], client_addr);
            else if (req_type == "UNP" && tokens.size() > 2) handle_unp(username, tokens[2], client_addr);
            else if (req_type == "SCH" && tokens.size() > 2) handle_sch(username, tokens[2], client_addr);
            else if (req_type == "GET" && tokens.size() > 2) handle_get(username, tokens[2], client_addr);
            else send_response("ERR Invalid command", client_addr);
        }
    }
